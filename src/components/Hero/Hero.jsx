...
const Hero = () => {
  ...
  return (
    <section id="home" className="relative overflow-hidden bg-gradient-to-b from-black to-gray-900 min-h-screen flex items-center justify-center pt-16 pb-20 lg:pt-20">
      ...
      {floatingItems.map((item, index) => (
        ...
        >
          <div className="relative group">
            ...
            <div className="relative px-3 py-3 md:px-4 md:py-4 bg-gray-900 rounded-lg leading-none flex items-center justify-center">
              {item.icon}
            </div>
          </div>
          <span className="mt-2 text-xs md:text-sm text-gray-300">{item.text}</span>
        </motion.div>
      ))}
      ...
      <div className="relative z-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto text-center">
        ...
        >
          ...
          <motion.p 
            className="text-base md:text-xl text-gray-300 max-w-3xl mx-auto mb-8 md:mb-10 px-2"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-3 md:gap-4 px-4">
            <Link to="/about" className="w-full sm:w-auto"> 
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="w-full px-6 py-3 md:px-8 md:py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-full font-medium text-base md:text-lg shadow-lg hover:shadow-purple-500/50 transition-all duration-300 flex items-center justify-center gap-2"
              >
            ...
            <Link to="/reels" className="w-full sm:w-auto">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="w-full px-6 py-3 md:px-8 md:py-3 bg-transparent border-2 border-purple-500 text-white rounded-full font-medium text-base md:text-lg hover:bg-purple-900 hover:bg-opacity-30 transition-all duration-300"
              >
                Explore Events
              </motion.button>
            </Link>
        ...
        >
          {[
            ...
            <motion.div 
              key={index}
              whileHover={{ y: -5 }}
              className="bg-gray-900 bg-opacity-60 backdrop-blur-sm rounded-xl p-4 md:p-6 border border-gray-800"
            >
